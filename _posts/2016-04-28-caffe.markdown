...
# 1) it is better to used the latest driver from http://www.nvidia.com/Download/index.aspx
# 2) if using AWS see the steps in the AWS section prior to installing the drivers
NVIDIA_INSTALLER=$(ls *NVIDIA*.run)
...
If using Nvidia GPUs, the appropriate driver's need to be installed. The following steps are based on [this](http://tleyden.github.io/blog/2014/10/25/cuda-6-dot-5-on-aws-gpu-instance-running-ubuntu-14-dot-04/) blog. Note that two reboots are required.
...
# (as of May 11, 2016 for the g2 instances with GRID K520 GPUs use the link below)
